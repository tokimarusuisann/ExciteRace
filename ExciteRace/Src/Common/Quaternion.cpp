...
{
    // à íuèÓïÒÇ…âÒì]èÓïÒÇîΩâfÇ≥ÇπÇÈwd
    // pos' = qÅEposÅEq(-1)
...
Quaternion Quaternion::SlerpUnclamped(Quaternion a, Quaternion b, float t)
{
    ...
    if (a.LengthSquared() == 0.0f)
    {
        ...
        {
            //óºï˚Ç∆Ç‡É[Éçélå≥êîÇ»ÇÁíPà élå≥êîÇï‘Ç∑
            return Identity();
        }
        //aÇ™É[Éçélå≥êîÇ»ÇÁbÇï‘Ç∑
        return b;
    }
    else if (b.LengthSquared() == 0.0f)
    { 
        //bÇ™É[Éçélå≥êîÇ»ÇÁaÇï‘Ç∑
        return a;
    }

    //aÇ∆bÇÃì‡êœÇãÅÇﬂë´ÇµçáÇÌÇπÇÈ
    float cosHalfAngle = (float)(a.w * b.w) + VDot(a.xyz(), b.xyz());

    //ì‡êœÇ™1Ç‹ÇΩÇÕ-1Ç»ÇÁï‚ä‘ïsóv
    if (cosHalfAngle >= 1.0f || cosHalfAngle <= -1.0f)
    {
        //äpìxÇ™0Ç»ÇÃÇ≈ÅAaÇÇªÇÃÇ‹Ç‹ï‘Ç∑
        return a;
    }
    else if (cosHalfAngle < 0.0f)
    {
        // bÇ™îΩëŒï˚å¸Çå¸Ç¢ÇƒÇ¢ÇÈèÍçáÇÕÅAbÇîΩì]Ç≥ÇπÇÈ
		b.x = b.x * -1.0f;
		b.y = b.y * -1.0f;
		b.z = b.z * -1.0f;
		b.w = -b.w;
